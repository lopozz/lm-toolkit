"""Interactive CLI client for exercising an LFM2.5-Audio server.

The script is intended as a lightweight end-to-end test and demonstration
client for the server's OpenAI-compatible streaming chat-completions API. It
supports three interaction patterns:

- ASR: send microphone recordings or WAV files and stream back transcription.
- TTS: send text and stream synthesized audio for immediate playback.
- Interleaved chat: exchange text and audio messages while preserving server
  context across turns, with support for explicitly resetting the conversation.

Set the checkpoint directory:

```
export CKPT="models/LFM2.5-Audio-1.5B-GGUF"
```

Launch the LFM2.5-Audio server:

```
llama.cpp/build/bin/llama-liquid-audio-server \
...
https://github.com/tdakhran/llama.cpp/blob/tarek/feat/os-lfm2.5-audio-1.5b-upstream/tools/liquid-audio/liquid_audio_chat.py
"""
...
def print_help():
    ...
    print(
        """
Commands:
  /mode <asr|tts|interleaved|audio-chat>    - Switch mode
...
def main():
    ...
    parser.add_argument(
        "--mode",
        type=str,
        choices=["asr", "tts", "interleaved", "audio-chat"],
        default="audio-chat",
        help="Initial mode",
    )
    parser.add_argument(
    ...
    while True:
        try:
            ...
            mode_indicator = {"asr": "[ASR]", "tts": "[TTS]", "interleaved": "[INT]", "audio-chat": "[AUDIO]"}
            ...
            if user_input.startswith("/"):
                ...
                if cmd in ("/quit", "/exit"):
                    ...
                    if arg in ("asr", "tts", "interleaved", "audio-chat"):
                        if arg == mode:
                    ...
                    if mode not in ("interleaved", "audio-chat"):
                        print("Reset only available in interleaved or audio-chat mode")
                        continue
                    is_first_message = True
                    print("Context reset")
                    continue
            ...
            try:
                ...
                if mode in ("asr", "tts"):
                    ...
                    AUDIO_CHAT_CONTEXT = """
Use the following information to answer the user's spoken questions.

Hotel information:
- The hotel is located at Piazza dei Fiori 3.
- Check in possible from 4PM

Answer in concise sentences.
If the answer is not available in the provided information, say that you do not know.
""".strip()
                    # audio chat mode: only send new messages
                    messages = []
                    ...
                    if is_first_message:
                        ...
                        messages.append(
                            {"role": "user", "content": AUDIO_CHAT_CONTEXT}
                        )
                ...
                _, _ = process_stream(stream, audio_player)
